...
/**
 * Formata um objeto Date para string DD/MM/AAAA de forma consistente,
 * independente do locale do browser.
 * @param {Date} data
 * @returns {string}
 */
 ...
 * Sempre reconstrói o mapa do zero — apagar uma linha na planilha
 * faz o horário voltar a ficar disponível automaticamente no próximo polling.
 * Chamado na inicialização e pelo polling a cada 30s.
 */
async function carregarCSV() {
  try {
    // cache-bust garante que o browser não use resposta antiga em cache
    ...
    // ── Reconstrói o mapa ZERADO a cada leitura ──────────────────────────
    // Isso garante que horários apagados na planilha somam da lista de ocupados.
    // Nunca acumula dados velhos entre chamadas.
    const novo = {};
    ...
    for (const linha of linhas) {
      ...
      const horario = cols[1]; // coluna B — formato HH:MM
      const data    = cols[2]; // coluna C — formato DD/MM/AAAA

      // Valida o formato esperado antes de inserir
    ...
    // Substitui o mapa inteiro — sem resíduo de dados antigos
    state.horariosOcupadosPorDia = novo;

    // Atualiza o alias do dia atualmente selecionado usando formatação consistente
    if (state.diaSelecionado) {
      const chave = formatarData(state.diaSelecionado);
      // Sempre recria o Set do novo mapa — nunca reutiliza referência antiga
...
async function selecionarDia(data, btn) {
  ...
  // Usa formatação manual para garantir DD/MM/AAAA em qualquer browser/OS
  ...
  // Usa os dados já carregados para renderização imediata
  state.horariosOcupados = state.horariosOcupadosPorDia[chave] ?? new Set();
  renderizarSlots();
...
function validarFormulario() {
  ...
  const dataSelecionada = state.diaSelecionado
    ? formatarData(state.diaSelecionado)
