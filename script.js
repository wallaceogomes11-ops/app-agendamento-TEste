...
async function sincronizarHorarios() {
  // Se estiver abrindo como arquivo local (file://), pula o fetch e renderiza direto
  if (location.protocol === 'file:') {
    console.info('[BarberSchedule] Modo local (file://) — sincronização desativada.');
    renderizarSlots();
    return;
  }
  ...
  // Timeout de 5s para não deixar a tela travada caso o Sheets demore
  const timeout = setTimeout(() => {
    console.warn('[BarberSchedule] Timeout — renderizando com estado local.');
    renderizarSlots();
  }, 5000);
