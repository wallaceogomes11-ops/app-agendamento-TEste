...
async function init() {
  ...
  // Busca horários ocupados na planilha antes de renderizar
  await sincronizarHorarios();
}
...
async function sincronizarHorarios() {
  ...
  try {
    ...
    // Substitui o Set inteiro pelo que veio da planilha
    state.horariosOcupados = new Set(ocupados);

  } catch (err) {
    // Falha silenciosa: mantém o estado local existente e segue em frente
    console.warn('[BarberSchedule] Não foi possível sincronizar horários:', err);
  }

  // Sempre renderiza — com dados da planilha ou com o estado local
  renderizarSlots();
}

/**
 * Sincroniza o estado com a planilha em background, sem mostrar skeleton
 * nem re-renderizar a grade inteira. Apenas remove da tela os horários
 * que outros usuários agendaram enquanto este estava no formulário.
 */
async function sincronizarEmBackground() {
  try {
    const ocupados = await buscarHorariosOcupados();
    const novoSet = new Set(ocupados);

    // Remove da tela apenas os slots que surgiram como ocupados desde a última sync
    novoSet.forEach(horario => {
      if (!state.horariosOcupados.has(horario)) {
        // Outro usuário agendou este horário — remove da tela silenciosamente
    ...
    state.horariosOcupados = novoSet;
    atualizarContadorSlots();

  } catch (err) {
    // Falha silenciosa — estado local já está correto para este usuário
    console.warn('[BarberSchedule] Sync em background falhou:', err);
  }
}
...
async function buscarHorariosOcupados() {
  // GET simples — sem no-cors porque precisamos LER a resposta
  const resposta = await fetch(CONFIG.SHEETS_ENDPOINT, {
    method: 'GET',
    // O Apps Script responde com CORS quando acessado via GET + redirect
    redirect: 'follow',
  });
  ...
  // Garante que é um array de strings, ignorando valores vazios
  if (!Array.isArray(ocupados)) return [];
  return ocupados.filter(h => typeof h === 'string' && h.trim() !== '');
}
...
async function confirmarAgendamento() {
  ...
  try {
    ...
    // Re-sincroniza silenciosamente em background (sem skeleton, sem re-render)
    // para capturar agendamentos feitos por outros usuários no mesmo período
...
/**
 * Envia os dados do agendamento para o Google Apps Script via GET + params.
 *
 * Por que GET em vez de POST?
 * O Apps Script bloqueia POST com CORS a menos que defina headers explícitos.
 * Já o GET via redirect funciona sem bloqueio e permite LER a resposta,
 * confirmando que o dado foi realmente gravado antes de atualizar a UI.
 * O Apps Script precisa tratar o parâmetro "acao=agendar" no doGet().
 *
 * @param {Object} dados
...
async function enviarParaSheets(dados) {
  ...
  const resposta = await fetch(`${CONFIG.SHEETS_ENDPOINT}?${params.toString()}`, {
    method:   'GET',
    redirect: 'follow',
  });
  ...
  if (resultado.status !== 'success') {
    throw new Error(resultado.mensagem || 'Resposta inesperada do servidor');
  }
}
