...
async function sincronizarHorariosOcupados() {
  try {
    ...
    const resposta = await fetch(url, {
      method: 'GET',
      cache: 'no-store', // Garante que não virá de cache do navegador
      redirect: 'follow', // Segue o redirect 302 que o Apps Script sempre emite
    });

    // Apps Script redireciona para uma URL diferente — resposta pode ter status 0 (opaque)
    // ou 200 após o follow. Só lança erro em falhas reais de rede.
    if (!resposta.ok && resposta.type !== 'opaqueredirect') throw new Error(`HTTP ${resposta.status}`);
 ...
 * 1. Valida localmente
 * 2. Checa rápido se já ocupado no estado local
 * 3. Envia ao backend — que re-valida antes de gravar (controle de concorrência)
 * 4. Processa resposta: 'ok' → confirma | 'conflito' → avisa e sincroniza
 */
async function confirmarAgendamento() {
  if (state.carregando) return; // Evita duplo envio

  const { valido, dados } = validarFormulario();
  if (!valido) return;

  // Checagem rápida local antes de ir ao servidor
  ...
  try {
    const resultado = await enviarAgendamento(dados);

    if (resultado.status === 'ok') {
      // Backend confirmou — atualiza localmente e na UI imediatamente
      ...
      // Outro usuário agendou o mesmo horário durante a requisição
      restaurarBotaoConfirmar();
      exibirToast('⚠️ Horário ocupado por outro cliente agora. Escolha outro.', 'error');
      fecharModal();
      await sincronizarHorariosOcupados(); // Atualiza imediatamente
...
/**
 * Envia os dados ao Apps Script via POST.
 * O backend verifica atomicamente se o horário ainda está livre antes de gravar.
 *
 * @param {Object} dados
 * @returns {Promise<{ status: 'ok'|'conflito'|'erro', mensagem?: string }>}
 */
async function enviarAgendamento(dados) {
  const resposta = await fetch(CONFIG.SHEETS_ENDPOINT, {
    method: 'POST',
    // Usamos 'text/plain' pois Apps Script aceita este content-type sem bloqueios CORS
    headers: { 'Content-Type': 'text/plain' },
    redirect: 'follow', // Segue o redirect 302 que o Apps Script sempre emite
    body: JSON.stringify({
      acao:        'agendar', // Campo que identifica a operação no backend
      nome:        dados.nome,
  ...
  // Só lança erro em falhas reais (rede, timeout) — não em redirects normais do Apps Script
  if (!resposta.ok && resposta.type !== 'opaqueredirect') throw new Error(`HTTP ${resposta.status}`);
